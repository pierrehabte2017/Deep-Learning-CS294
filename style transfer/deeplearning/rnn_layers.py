...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    x, Wx, prev_h, Wh, tmp = cache
    act_der = (1.0 - np.tanh(tmp) ** 2) * dnext_h
    # Backprop
    dx = np.dot(act_der, Wx.T)
    dWx = np.dot(x.T, act_der)
    dprev_h = np.dot(act_der, Wh.T)
    dWh = np.dot(prev_h.T, act_der)
    db = np.sum(act_der, axis=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dh_ag = dh[:, t, :] + dprev_h
        dx_step, dprev_h, dprev_c, dWx_step, dWh_step, db_step = lstm_step_backward(dh_ag, dprev_c,
                                                                                    forward_steps_cache[t])
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
        dx[:, t, :] = dx_step
